...
#ifndef EXTEND_STANDARD_PARTICLES_INCLUDED
#define EXTEND_STANDARD_PARTICLES_INCLUDED
...
struct Input
{
    ...
    #endif
    #if defined(SOFTPARTICLES_ON) || defined(_FADING_ON) || defined(_ALPHABUFFER)
...
half4 fragParticleUnlit (VertexOutput IN) : SV_Target
{
    ...
    { 
        ...
        float sceneZ = LinearEyeDepth (SAMPLE_DEPTH_TEXTURE_PROJ(_CameraDepthTexture, UNITY_PROJ_COORD(IN.projectedPosition))); 
		#endif 
        softParticlesFade = saturate (SOFT_PARTICLE_INV_FADE_DISTANCE * ((sceneZ - SOFT_PARTICLE_NEAR_FADE) - IN.projectedPosition.z)); 
        ALBEDO_MUL *= softParticlesFade; 
    }
#else
    float softParticlesFade = 1.0f;
